...
export class Migrations<T extends VersionedObject> {
  ...
  run(object: T): T {
    ...
    assert(
      ...
      `Object is starting at version ${lastVersion}, which is already past the target version (${this.targetVersion})`,
    );
    while (lastVersion !== this.targetVersion) {
      ...
      if (lastVersion) {
        assert(
          ...
          `Migrating version ${lastVersion} resulted in version ${newVersion}. That is not an upgrade.`,
        );
      }
      assert(
