...
} from '../ng-vitest/provide-persistence.harness';
import {
  PersistenceCodec,
  PersistenceConfig,
  providePersistence,
...
describe('providePersistence()', () => {
  ...
  > extends AngularContext {
    ...
    // persistence!: PersistenceHarness<P>;
    ...
    constructor(config: Partial<PersistenceConfig<CounterState, P>> = {}) {
      ...
      super({
        providers: [
          ...
          provideMockPersistence(() => this.initialState),
        ],
      });
    }
    ...
    protected override async init(): Promise<void> {
      await super.init();
      await this.tick();
      // this.persistence = new PersistenceHarness<P>(this, 'theKey');
    }
  }
